...
import { ArrowLeft, Droplets, Camera, MapPin, CheckCircle, XCircle, Save } from 'lucide-react';
...
interface LubricationCheckItem {
  id: string;
  category: string;
  item: string;
  description: string;
...
const Lubrication: React.FC = () => {
  ...
  const [checkItems, setCheckItems] = useState<LubricationCheckItem[]>([]);
  ...
  const initializeCheckItems = () => {
    const items: LubricationCheckItem[] = [
      { id: 'lub_1', category: '슬라이드', item: '슬라이드 작동 확인', description: '슬라이드 작동 시 걸림 여부 및 유격 확인', status: null, notes: '' },
      { id: 'lub_2', category: '슬라이드', item: '슬라이드 습합 상태', description: '슬라이드 표면 습합제 도포 상태', status: null, notes: '' },
      { id: 'lub_3', category: '슬라이드', item: '슬라이드 마모 확인', description: '슬라이드 표면 마모 및 손상 여부', status: null, notes: '' },
      { id: 'lub_4', category: '리프터', item: '리프터 작동 확인', description: '리프터 상하 작동 원활성 확인', status: null, notes: '' },
      { id: 'lub_5', category: '리프터', item: '리프터 습합 상태', description: '리프터 가이드부 습합 상태', status: null, notes: '' },
      { id: 'lub_6', category: '가이드핀', item: '가이드핀 유격', description: '가이드핀과 부시 간 유격 확인', status: null, notes: '' },
      { id: 'lub_7', category: '가이드핀', item: '가이드핀 습합', description: '가이드핀 표면 습합제 도포 상태', status: null, notes: '' },
      { id: 'lub_8', category: '가이드핀', item: '가이드핀 마모', description: '가이드핀 표면 마모 및 스크래치', status: null, notes: '' },
      { id: 'lub_9', category: '에젝터', item: '에젝터 핀 작동', description: '에젝터 핀 작동 시 걸림 여부', status: null, notes: '' },
      { id: 'lub_10', category: '에젝터', item: '에젝터 습합 상태', description: '에젝터 핀 표면 습합 상태', status: null, notes: '' },
      { id: 'lub_11', category: '기타', item: '코어 핀 습합', description: '코어 핀 표면 습합 상태', status: null, notes: '' },
      { id: 'lub_12', category: '기타', item: '인서트 핀 습합', description: '인서트 핀 표면 습합 상태', status: null, notes: '' }
    ];
    setCheckItems(items);
  };
  ...
  const groupedItems = checkItems.reduce((acc, item) => {
    ...
    return acc;
  }, {} as Record<string, LubricationCheckItem[]>);
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors disabled:bg-slate-300"
              >
                ...
                <div key={item.id} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <button className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-green-100 transition-colors">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-colors font-semibold flex items-center gap-2"
          >
